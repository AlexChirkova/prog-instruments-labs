...
def show_emp_and_theor_chi2_distrib_density(sample: np.array, df: int = 0) -> None:
    ...
    plt.hist(sample, bins='scott', density=True,
...
def show_emp_and_theor_chi2_distrib_func(sample: np.array, df: int = 0) -> None:
    ...
    plt.plot(theoretical_y, theoretical_cdf_y, 'r-', linewidth=2,
             label=f'Теоретическая χ²({df})')
...
def boxplot_mean_var(sample: np.array) -> None:
    ...
    plt.boxplot(sample, vert=True, patch_artist=True,
...
def main():
    ...
    print(f"Теоретическкая дисперсия: {2 * sigma ** 4 * (size_of_sample - 1) / size_of_sample ** 2}")
    ...
    repetitions = 550
    sample_mean_vars = create_sample_norm_vars(repetitions, size_of_sample, count_of_samples, a, sigma)
